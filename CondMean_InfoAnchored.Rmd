...
The method follows a proposal by @lu2021alternative. They suggested to obtain the variance for reference-based imputation methods by decoupling variance estimation from treatment effect estimation. To achieve this, they replace the reference-based imputation by a MAR imputation combined with a delta-adjustment where delta is selected in a data-driven way to match the reference-based estimator. In the following, we show that this can be easily implemented using `rbmi`. 
...
We use the function `analyse()` to add the delta-adjustment and perform the analysis of the imputed datasets. `analyse()` will take as the input argument `imputations = impute_obj_MAR`, i.e. the imputation object corresponding to the MAR imputation (and not the JR imputation!) and will add the delta adjustment `delta = delta_adjust` prior to the analysis.
...
get_delta_match_refBased <- function(draws, data_ice, references) {
  ...
  vars <- draws$data$vars
  
  # Impute under defined imputation strategy
  res_imp <- impute(
    draws = draws,
...
delta_adjust_fun <- get_delta_match_refBased(drawObj, dat_ice, references)
head(delta_adjust_fun)

identical(delta_adjust, delta_adjust_fun)
```
